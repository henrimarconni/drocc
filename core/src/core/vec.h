...
#define vec_grow(vec) \
  (\
    ((vec).m = ((vec).m ? (vec).m * 2 : 2) ) ,\
    (vec).get = realloc((vec).get,\
    (vec).m * sizeof(*(vec).get)),\
    0\
  )
...
#define vec_remove_swap(vec, i)                                              \
  (                                                                          \
    assert((i) < (vec).n),                                                   \
    (vec).get[(i)] = (vec).get[(vec).n - 1],                                 \
    --(vec).n,                                                               \
    0                                                                        \
  )
/// Resize vector so that vec.m = vec.n (capacity = length)
#define vec_freeze(vec) \
  (\
    (vec).get = realloc((vec).get,\
    (vec).n * sizeof(*(vec).get)),\
    0\
  )
